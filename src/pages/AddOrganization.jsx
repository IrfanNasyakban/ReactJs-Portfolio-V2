...
const AddOrganization = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.organisasi.trim()) {
      newErrors.organisasi = 'Organization name is required';
    }

    if (!formData.divisi.trim()) {
      newErrors.divisi = 'Division/Position is required';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('organisasi', formData.organisasi);
      ...
      await axios.post(`${apiUrl}/organizations`, formDataToSend, {
        headers: { 
      ...
      alert(error.response?.data?.message || 'Failed to add organization');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                Add your organizational activities to showcase your leadership
              </p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <form onSubmit={handleSubmit}>
            <div className={`rounded-xl border p-8 ${isDark ? 'bg-[#282C33] border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className="space-y-6">
                ...
                >
                  ...
                  <input
                    ...
                    placeholder="e.g., Student Council, IEEE, Rotaract Club"
                  ...
                  />
                  {errors.organisasi && (
                    <p className="mt-1 text-sm text-red-500">{errors.organisasi}</p>
                  )}
                </motion.div>
                ...
                >
                  <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    <div className="flex items-center gap-2 mb-2">
                      <FaUserTie className={isDark ? 'text-gray-400' : 'text-gray-500'} />
                      Division / Position
                      <span className="text-red-500">*</span>
                    </div>
                  </label>
                  ...
                  />
                  {errors.divisi && (
                    <p className="mt-1 text-sm text-red-500">{errors.divisi}</p>
                  )}
                </motion.div>
                ...
                >
                  <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    <div className="flex items-center gap-2 mb-2">
                      <FaMapMarkerAlt className={isDark ? 'text-gray-400' : 'text-gray-500'} />
                      Location (Optional)
                    </div>
                  </label>
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="e.g., Jakarta, Online, University Campus"
                    className={`w-full px-4 py-3 rounded-lg border transition-colors ${
                      isDark 
                        ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' 
                        : 'bg-white border-gray-300 text-gray-800 placeholder-gray-400'
                ...
                >
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="e.g., 2020 - 2022, Jan 2021 - Present"
                    className={`w-full px-4 py-3 rounded-lg border transition-colors ${
                      isDark 
                        ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' 
                        : 'bg-white border-gray-300 text-gray-800 placeholder-gray-400'
                  ...
                  <p className={`mt-1 text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                    Enter the duration of your involvement in the organization
                  </p>
                </motion.div>
                ...
                >
                  ...
                  <p className={`mt-1 text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                    Upload the organization's logo for better visual presentation
                  </p>
                </motion.div>
              </div>
              ...
              >
                ...
                >
                  <FaSave />
                  <span>{loading ? 'Saving...' : 'Save Organization'}</span>
                </button>
                <button
                  type="button"
          ...
          >
            <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
              <strong>Tip:</strong> Include all your organizational activities including clubs, volunteer work, and community involvement. This demonstrates your teamwork and leadership skills.
            </p>
          </motion.div>
        </motion.div>
      </div>
