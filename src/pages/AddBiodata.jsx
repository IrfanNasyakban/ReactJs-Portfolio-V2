...
const AddBiodata = () => {
  ...
  const saveBiodata = async (e) => {
    ...
    if (!tglLahir) newErrors.tglLahir = 'Date of birth is required';
    if (!noHp.trim()) newErrors.noHp = 'Phone number is required';
    if (!alamat.trim()) newErrors.alamat = 'Address is required';
    if (!file) newErrors.image = 'Profile photo is required';
    ...
    try {
      ...
      console.log('Submitting biodata...');
      ...
      if (error.response?.data?.msg) {
        ...
        setErrors({ submit: 'Failed to create biodata. Please try again.' });
      }
    } finally {
      setLoading(false);
    }
