    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className={`text-lg ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Loading certificates...</p>
        </div>
      </div>
    );
  }
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>#certificates</h1>
                <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Manage your certifications</p>
              </div>
            </div>
            <button
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          {filteredCertificates.length === 0 ? (
            <div className={`text-center p-12 rounded-xl border ${isDark ? 'bg-[#282C33] border-gray-700' : 'bg-white border-gray-200'}`}>
              <FaCertificate className="text-6xl mx-auto mb-4 opacity-50" style={{ color: safeColor }} />
              <h3 className={`text-xl font-bold mb-2 ${isDark ? 'text-white' : 'text-gray-800'}`}>No Certificates Found</h3>
                ...
                <table className="w-full">
                  <thead>
                    <tr className={isDark ? 'bg-gray-700' : 'bg-gray-50'}>
                      ...
                      <th className={`px-6 py-4 text-left text-xs font-medium uppercase tracking-wider ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Certificate</th>
                      <th className={`px-6 py-4 text-left text-xs font-medium uppercase tracking-wider ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Description</th>
                      <th className={`px-6 py-4 text-left text-xs font-medium uppercase tracking-wider ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Link</th>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div 
                              className={`w-16 h-16 rounded-lg items-center justify-center ${cert.url ? 'hidden' : 'flex'}`}
                              style={{ backgroundColor: getColorWithOpacity(safeColor, 0.1) }}
                            >
                              <FaImage style={{ color: safeColor }} />
                            </div>
                        ...
                        <td className={`px-6 py-4 text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                          <div className="max-w-xs">
                            {cert.deskripsi ? (
                              cert.deskripsi.length > 80 
                                ? `${cert.deskripsi.substring(0, 80)}...` 
                                : cert.deskripsi
                            ) : '-'}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              onClick={() => navigate(`/edit-certificates/${cert.id}`)}
