...
const EditEducation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(`${apiUrl}/education/${id}`, formDataToSend, {
        headers: { 
      ...
      alert(error.response?.data?.message || 'Failed to update education');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className={`text-lg ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Loading education data...</p>
        </div>
      </div>
    );
  }
          ...
          <form onSubmit={handleSubmit}>
            <div className={`rounded-xl border p-8 ${isDark ? 'bg-[#282C33] border-gray-700' : 'bg-white border-gray-200'}`}>
              ...
              >
                ...
                >
                  <FaSave />
                  <span>{loading ? 'Updating...' : 'Update Education'}</span>
                </button>
                <button
                  type="button"
